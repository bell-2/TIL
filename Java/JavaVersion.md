...
LTS(Long Term Support)는 말 그대로 장기 지원되는 버전을 의미합니다.
...
위에서 잠깐 언급한 JDK는 그럼 무엇일까요? 그리고 JRE란?

개발 환경을 구성할 때마다 많이 듣는 단어지만, 그게 뭔데?라고 누군가 물어봤을 때 정확히 대답할 수 없는 이 씁쓸함.. 달래봅시다

![JDK&JRE](/Image/jdk.png)

## JDK (Java Developent Kit)

Java를 사용하기 위해 기능을 갖고 있는 Java용 SDK(Software Development Kit) 입니다.

프로그램에 필요한 JVM(자바 가상 기계), API, 라이브러리, 컴파일러 등의 개발 도구가 포함된 소프트웨어 패키지 입니다.

즉, 프로그램을 생성/실행/컴파일을 해주는 녀석입니다.

![JDK&JRE](/Image/jdk2.png)

Intellij에서 Application을 실행시킬 때도 SDK를 선택해주잖슴? JDK는 프로그램 실행 시에 꼭 필요하단 겁니다.

### 구성 요소
1. 자바 컴파일러(javac): Java 소스 코드를 컴파일하여 바이트 코드로 변환하는 도구
2. Java 런타임 환경 (JRE): Java 어플리케이션을 실행하기 위한 라이브러리와 기타 파일들을 제공
3. Java 개발 도구(javap, jdb 등): 디버깅, 코드 분석 등의 작업을 수행하는 도구들이 포함됨
4. Java 라이브러리 클래스: 다양한 기능을 제공하는 Java 클래스 라이브러리가 포함됨. ex) 데이터 구조, 네트워킹, 파일 처리
5. 문서 및 예제 코드: JDK에는 Java 개발에 대한 문서와 예제 코드가 포함되어있츰.

JDK 설치를 하면, Java 프로그램을 개발하고 실행하는데 필요한 모든 것이 있기 때문에, Java 개발자들은 JDK를 설치하여 개발 환경을 구축할 수 있습니다. 

위에서 다룬 Java Version에 따라 추가된 기능과 변경 사항이 있을 수 있답니다.

## JRE (Java Runtime)
자바 프로그램 실행에 필요한 자바 가상 기계(JVM), 라이브러리 API가 포함된 소프트웨어 패키지

### 구성 요소
1. 자바 가상 머신 (JVM): Java 어플리케이션 실행 시 사용되는 가상 머신. java 바이트 코드를 해당 플랫폼의 네이티브 코드로 변환하고 실행해줌 
2. Java 클래스 라이브러리: 데이터 구조/입출력/네트워크 등의 다양한 기능을 제공하는 클래스들 포함

JRE는 따로 설치할 필요는 없고, JDK에 포함되기 때문에 JDK를 설치하면 다 됩니다 🙂 
